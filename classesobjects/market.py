...
class Transaksi :
    def __init__(self, nama, totalHarga) :
        self.nama = nama
...
def kosongkanCartDanCatatTransaksi(nama, totalHarga) :
    for item in cart :
        listBuah[item['index']]['stock'] -= item['qty']
    cart.clear()
    listTransaksi.append(Transaksi(nama, totalHarga))
...
def membeliBuah() :
    ...
    while True :
        ...
        if(jmlUang > totalHarga) :
            ...
            kosongkanCartDanCatatTransaksi(nama, totalHarga)
            break
...
def lihatRiwayatTransaksi() :
    print('Riwayat Transaksi :\n')
    print('Nama\t| Total Harga')
    for trx in listTransaksi :
        print('{}\t| {}'.format(trx.nama, trx.totalHarga))
